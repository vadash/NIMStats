...
- Signature: `computeHourlyStats(runs, models?)` — optional `models` allowlist (Set or array); when omitted/empty, all models are aggregated. The Best Timeslot card passes the user-selected subset of the top-10 leaderboard models.
- **Metric is token speed** (`tokensGenerated / (responseTime/1000)`, t/s), not response time — higher is better. A failed call contributes `penalty = 0` t/s. `computeBestTimeslots()` scores zones as `avgTps / maxAvg * 100` and returns `avgTps` per zone; the chart's color ramp is inverted accordingly (green = fast).
...
- Returns `{ hourAvg, smoothed, isReal, hourRealCount, weightedCounts, hourFailCount }` — all 24-length folded arrays of t/s. `computeBestTimeslots()` gates zones on `realCount >= 2` with a cascade fallback (`2 → 1 → 0`) so sparse data still renders medals instead of a blank chart. The fallback must check whether a valid **non-overlapping 3-zone combo exists**, not just the pool size — a contiguous pool of 4 zones has size ≥3 but zero possible combos.
